...
G = SymmetricProductFun(fK,Chebyshev(d),JacobiWeight(-.5,-.5,Chebyshev(d)))
G1 = SymmetricProductFun(fK,Ultraspherical{1}(d),JacobiWeight(-.5,-.5,Chebyshev(d)))
G2 = SymmetricProductFun(fK,Ultraspherical{1}(d),JacobiWeight(-.5,-.5,ApproxFun.ChebyshevDirichlet{1,1}(d)))
...
G = ProductFun(f2,Fourier(),Fourier())
...
G = ProductFun(f2,Laurent(),Laurent())
@test norm(f2[.456-.123]-G[.123,.456])≤2eps()
...
SymmetricProductFun(fK,Chebyshev(d),Chebyshev(d))
@time G = SymmetricProductFun(fK,Chebyshev(d),Chebyshev(d))
SymmetricProductFun(fK,Ultraspherical{1}(d),ApproxFun.ChebyshevDirichlet{1,1}(d))
@time G = SymmetricProductFun(fK,Ultraspherical{1}(d),ApproxFun.ChebyshevDirichlet{1,1}(d))
println("Chebyshev addition: Time should be ~0.01 seconds.")

f2 = Fun(θ->besselj0(500*abs(2sin(θ/2))),CosSpace())
ProductFun(f2,Fourier(),Fourier())
@time G = ProductFun(f2,Fourier(),Fourier())
println("CosSpace addition: Time should be ~0.05 seconds.")

f2 = Fun(θ->besselj0(500*abs(2sin(θ/2))),Laurent())
ProductFun(f2,Laurent(),Laurent())
@time G = ProductFun(f2,Laurent(),Laurent())
