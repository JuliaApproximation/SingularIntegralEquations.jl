...
function integrand!{T<:Union{Float64,Complex{Float64}}}(u::Vector{Complex{Float64}}, ux::Vector{Complex{Float64}}, uy::Vector{Complex{Float64}}, s::Vector{T}, a::Float64, b::Float64, x::Float64, y::Float64, derivs::Bool)
    # Precalculations
    es = exp(s)
    ies = 1.0./es
    u[:] = cis(a * ies + b * es - es .* es .* es / 12.0)
